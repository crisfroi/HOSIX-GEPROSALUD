...
import { useHosixEspecialidades, EspecialidadMedica } from '@/hooks/useHosixEspecialidades'
import { Checkbox } from '@/components/ui/checkbox'

export function EspecialidadesMedicasManager() {
  const { especialidades, cargando, crear, actualizar, eliminar } = useHosixEspecialidades()
  const { toast } = useToast()
  const [open, setOpen] = useState(false)
  const [editando, setEditando] = useState<EspecialidadMedica | null>(null)
  ...
  const resetForm = () => {
    setFormData({ codigo: '', nombre: '', descripcion: '', area: '', activo: true })
    setEditando(null)
  }

  const handleEdit = (especialidad: EspecialidadMedica) => {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editando ? 'Editar' : 'Nueva'} Especialidad</DialogTitle>
            </DialogHeader>
            ...
            <TableBody>
              {especialidades.map((esp) => (
                <TableRow key={esp.id}>
                  ...
                  <TableCell>{esp.area || '-'}</TableCell>
                  <TableCell>{esp.activo ? '✓ Activo' : 'Inactivo'}</TableCell>
